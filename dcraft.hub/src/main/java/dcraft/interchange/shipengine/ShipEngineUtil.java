...
public class ShipEngineUtil {
	...
	static public void checkRates(String alt, RecordStruct order, OperationOutcomeRecord callback) {
	...
	static public void createLabel(String alt, RecordStruct order, String carrier, String shipService, OperationOutcomeRecord callback) {
		XElement auth = ApplicationHub.getCatalogSettings("CMS-Shipping-ShipEngine", alt);
		...
		RecordStruct shipment = RecordStruct.record()
				.with("shipment", RecordStruct.record()
						.with("carrier_id", carrier)
						.with("service_code", shipService)
						.with("ship_from",
								RecordStruct.record()
										.with("name", shipfrom.getAttribute("Name"))
										.with("company_name", shipfrom.getAttribute("Company"))
				...
				.with("label_format", "pdf")
				.with("test_label", ! ApplicationHub.isProduction());

		System.out.println("ShipEngine request:\n" + shipment.toPrettyString());

		try {
			OperationContext.getOrThrow().touch();

			URL url = new URL(ShipEngineUtil.ENDPOINT + "v1/labels");
