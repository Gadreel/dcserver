...
public class ObjectBuilder implements ICompositeBuilder {
	...
	private void endField(CompositeStruct child) throws BuilderStateException {
		...
		((RecordStruct)this.cstate.CurrentE).with(fi.CurrentField, (child != null) ? child : fi.CurrentValue);
	...
	private void endItem(CompositeStruct child) throws BuilderStateException {
		...
		// end the item
		if (!this.cstate.ValueComplete) {
			((ListStruct)this.cstate.CurrentE).withItem((child != null) ? child : this.cstate.CurrentValue);
	...
	public ICompositeBuilder rawValue(Object value) throws BuilderStateException {
		...
		if (value instanceof CharSequence)		
			this.cstate.CurrentValue = CompositeParser.parseJson((CharSequence)value);
		else if (value instanceof Memory)		
			this.cstate.CurrentValue = CompositeParser.parseJson((Memory)value);
