...
#
************************************************************************ */
package dcraft.scriptold.inst.ext;
	...
	public void run(final StackEntry stack) throws OperatingContextException {
		...
		for (int i = 0; i < params.size(); i++) {
			...
			if (StringUtil.isEmpty(v)) {
				...
				stack.setState(ExecuteState.Done);		// done is fine, the scriptold should decide what to do with the rrror
				stack.resume();
			}
		...
		try {
			...
			while ((line = input.readLine()) != null) {
				...
				if (code < 300)
					...
					Logger.error(400000 + code, line);		// TODO configure error start code
			}
			...
			if (handle != null) 
	            stack.addVariable(handle, IntegerStruct.of(ecode));
		} 
		catch (IOException x) {
