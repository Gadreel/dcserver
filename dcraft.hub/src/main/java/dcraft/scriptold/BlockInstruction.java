...
public abstract class BlockInstruction extends Instruction {
	...
	public boolean compile() {
        ...
        for (XElement child : this.source.selectAll("*")) {
        	...
        	/* cleaning up ---
            Instruction ni = ScriptHub.createInstruction(child);
            
            if (ni == null) {
            	Logger.errorTr(509, child.getName());
            	return false;
            }
    ...
    public void run(StackEntry stack) throws OperatingContextException {
		...
		this.alignInstruction(stack, new OperationOutcomeEmpty() {			
			@Override
						...
						if (child == null) {
							...
							if ((cstate != ExecuteState.Ready) && (cstate != ExecuteState.Resume)) {
						        if (cstate == ExecuteState.Break)  
						        	...
						        	BlockInstruction.this.nextInstruction(stack, new OperationOutcomeEmpty() {										
										@Override
										public void callback() throws OperatingContextException {
											stack.resume();
										}
    ...
    public void nextInstruction(final StackEntry stack, OperationOutcomeEmpty callback) throws OperatingContextException {
		final StackBlockEntry bstack = (StackBlockEntry)stack;
    ...
    public void alignInstruction(final StackEntry stack, OperationOutcomeEmpty callback) throws OperatingContextException {
