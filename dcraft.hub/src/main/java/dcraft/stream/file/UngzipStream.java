...
public class UngzipStream extends TransformFileStream {
    ...
    public void initializeFileValues(FileDescriptor src) {
		...
		if (StringUtil.isNotEmpty(this.nameHint)) 
			this.currfile.withPath("/" + this.nameHint);
		else if (StringUtil.isNotEmpty(src.getName()))
			this.currfile.withPath("/" + GzipUtils.getUncompressedFilename(src.getName()));
		else
			this.currfile.withPath("/" + FileUtil.randomFilename("bin"));
		
		this.currfile.withModificationTime(src.getModificationAsTime());
		// TODO this.currfile.setPermissions(src.getPermissions());
    }

    // return true when completely done
    protected void inflate(ByteBuf in) throws OperatingContextException {
        ...
        case HEADER_START:
            ...
            if (magic0 != 31) {
            	OperationContext.getAsTaskOrThrow().kill("Input is not in the GZIP format");
                return;
            }
            ...
            if (method != Deflater.DEFLATED) {
            	OperationContext.getAsTaskOrThrow().kill("Unsupported compression method " + method + " in the GZIP header");
            	return;
            }
            ...
            if ((this.flags & FRESERVED) != 0) {
            	OperationContext.getAsTaskOrThrow().kill("Reserved flags are set in the GZIP header");
                return;
            }
        ...
        case PROCESS_FHCRC:
            if ((this.flags & FHCRC) != 0) {
                ...
                if (crcValue != readCrc) {
                	OperationContext.getAsTaskOrThrow().kill("CRC value missmatch. Expected: " + crcValue + ", Got: " + readCrc);
                    return;
                }
            }
        ...
        case PRROCESS_CONTENT:
            ...
            ByteBuf decompressed = ApplicationHub.getBufferAllocator().heapBuffer(maxOutputLength);
            ...
            try {
                while (!this.inflater.needsInput()) {
                    ...
                    if (length == 0) {
                        ...
                        decompressed = ApplicationHub.getBufferAllocator().heapBuffer(maxOutputLength);
                    ...
                    if (outputLength > 0) {
                        ...
                        if (this.inflater.needsDictionary()) {
                            if (this.dictionary == null) {
                            	OperationContext.getAsTaskOrThrow().kill("decompression failure, unable to set dictionary as non was specified");
                            	return;
                            }
            	...
            	OperationContext.getAsTaskOrThrow().kill("decompression failure: " + x);
            	return;
            } 
            finally {
        ...
        case PROCESS_FOOTER:
            ...
            if (dataLength != readLength) {
            	OperationContext.getAsTaskOrThrow().kill("Number of bytes mismatch. Expected: " + dataLength + ", Got: " + readLength);
            	return;
            }
