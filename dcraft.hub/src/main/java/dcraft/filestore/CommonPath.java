...
public class CommonPath implements IDataExposer {
    ...
    public CommonPath(CharSequence pathname) {
        if (StringUtil.isEmpty(pathname) || (pathname.charAt(0) != '/'))
        	throw new IllegalArgumentException("Path not valid content: " + pathname);
    	
        this.path = CommonPath.normalizeAndCheck(pathname).toString();
    ...
    static public CharSequence normalizeAndCheck(CharSequence input) {
        int n = input.length();
    ...
    protected static CharSequence normalize(CharSequence input, int len, int off) {
        if (len == 0)
            return input;
