...
public class KeyUtil {
	...
	static public Collection<CertSummary> loadKeystoreSummary(Path keystore, char[] password) {
		KeyStore ks = null;
        ...
        try {
        	...
        	Logger.error("Unable to open/read keystore file: " + x);
        	return null;
        }
        ...
        try {
            ...
            while (aliases.hasMoreElements()) {
              ...
              if (encoded == null) {
            	  Logger.error("Unable to decode: " + summ.alias);
            	  return null;
              }
        	...
        	Logger.error("Unable to decode keystore file: " + x);
        	return null;
        }
	}
    ...
    if ((key instanceof PrivateKey) && "PKCS#8".equals(key.getFormat())) {
      ...
      try (FileOutputStream os = new FileOutputStream(alias + ".key")) {	-- don't use fos
	...
	public static String getCertThumbprint(X509Certificate cert) {
		try {
			...
			Logger.error("Unable to read certificate thumbprint: " + x);
