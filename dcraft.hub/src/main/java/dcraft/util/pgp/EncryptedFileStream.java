...
import dcraft.util.pgp.PgpUtil;
...
public class EncryptedFileStream {
    ...
    public void loadPublicKey(Path keyring) throws IOException, PGPException {
        ...
        InputStream keyIn = new BufferedInputStream(Files.newInputStream(keyring));
        
        // TODO does this close? review and cleanup
    ...
    public void allocNextBuffer(int size) {
		...
		this.out = ApplicationHub.getBufferAllocator().heapBuffer(Math.max(size, 32 * 1024));		// TODO config 
    }
    ...
    public void writeData(byte[] bytes, int offset, int len) {
    	...
    	if (pbb != null) {
    		...
    		while ((bbremaining > 0) && ((avail > 0) || (bbremaining + remaining) >= MAX_PACKET_SIZE)) {
        		...
        		if (bbremaining == 0) {
        			...
        			ByteBuf npb = ApplicationHub.getBufferAllocator().heapBuffer(MAX_PACKET_SIZE);
    	...
    	if (remaining > 0) {
    		if (this.packetbuf == null)
    			this.packetbuf = ApplicationHub.getBufferAllocator().heapBuffer(MAX_PACKET_SIZE);
