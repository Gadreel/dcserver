...
import dcraft.util.pdf.LineListener.LLResult;

/*
 * based on https://github.com/ralfstuckert/pdfbox-layout/blob/a21682d503aab8f5207ed01e7876bcbefd7f2721/src/main/java/rst/pdfbox/layout/text/TextSequenceUtil.java
 */
public class TextSequenceUtil {
    ...
    // dCA
    public static void drawText(TextSequence text,
    	    LineListener lineListener, Position upperLeft,
    	    DrawListener drawListener, Alignment alignment, float maxWidth,
    	    final float lineSpacing, final boolean applyLineSpacingToFirstLine)
    	    throws IOException {
    	List<TextLine> lines = wordWrapToLines(text, maxWidth);
    	...
    	for (int i = 0; i < lines.size(); i++) {
    	    ...
    	    LLResult lres = lineListener.prep(textLine, position, currentLineHeight);
    	    ...
    	    textLine.drawText(lres.stream, position, alignment, drawListener);
    	}
    }

    /*
    public static float calcTextHeight(TextSequence text, float maxWidth) throws IOException {
    	List<TextLine> lines = wordWrapToLines(text, maxWidth);
    	float height = 0;
