...
public class BasicSettingsObfuscator implements ISettingsObfuscator {
	...
	@Override
	public XElement configure(String id, String feed) {
		XElement clock = XElement.tag("Clock")
				.withAttribute("TimerClass", this.getClass().getCanonicalName())
				.withAttribute("Id", id)
				.withAttribute("Feed", feed);

		if (StringUtil.isEmpty(id)) {
			byte[] idbuff = new byte[64];
			RndUtil.random.nextBytes(idbuff);
			clock.withAttribute("Id", HexUtil.bufferToHex(idbuff));
		}
		
		if (StringUtil.isEmpty(feed)) {
			byte[] feedbuff = new byte[64];
			RndUtil.random.nextBytes(feedbuff);
			clock.withAttribute("Feed", HexUtil.bufferToHex(feedbuff));
		}
	...
	@Override
	public boolean checkHashPassword(CharSequence candidate, CharSequence hashed) {
