...
public class Memory implements IReader {
    ...
    public int readChar() {
    	...
    	try {
			...
			Logger.error("MEMORY could not read char: " + x);
    	}
    ...
    public CharSequence readCharsCapped(int bytes) {
    	...
    	try {
			...
			Logger.error("MEMORY could not read chars: " + x);
	 ...
	 */
	public boolean write(CharSequence str) {
		if (StringUtil.containsRestrictedChars(str)) {
			Logger.error("MEMORY got a bad string");		
			return false;
		}
		
		// TODO this could be more efficient - encode <= 64 bytes at a time and add
		return (this.write(Utf8Encoder.encode(str)) > 0);
	}
	
	public boolean writeLine(CharSequence str) {
		if (StringUtil.containsRestrictedChars(str)) {
			Logger.error("MEMORY got a bad string");		
			return false;
		}
    ...
    public long copyToStream(OutputStream stream, long count) {
		...
		try {
			...
			Logger.error("MEMORY got a bad stream: " + x);		
			return 0;
		}
    }
}
