...
public class QueryFirst extends RecordStruct {
	@Override
	public ReturnOption operation(StackWork state, XElement code) throws OperatingContextException {
		...
		if ("Where".equals(code.getName())) {
			...
			QueryFirst.addWhere(where.getFieldAsList("Children"), state, code);
					...
					.withOutcome(
						new OperationOutcomeStruct() {
							...
							public void callback(BaseStruct result) throws OperatingContextException {
								if (result instanceof ListStruct) {
									if (((ListStruct) result).size() > 0)
										result = ((ListStruct) result).getItem(0);
									else
