...
public class DayWindow {
	 ...
	 * @return
	 */
	public LocalTime nextTimeOn(ZonedDateTime si) {
		...
		int sidx = sil.getHour() * 60 + sil.getMinute() + 1;
	...
	public CheckLimitResult check(ZonedDateTime si) {
		if (this.excludeAll())
			return CheckLimitResult.Fail;
		
		LocalTime sil = si.toLocalTime();			
		return this.check(sil.getHour() * 60 + sil.getMinute());
	}
