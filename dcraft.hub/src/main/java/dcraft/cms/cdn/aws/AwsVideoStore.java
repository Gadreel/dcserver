...
public class AwsVideoStore extends AwsStore {
	// use with connect
	static public AwsVideoStore awsVideo() {
		AwsVideoStore driver = new AwsVideoStore();
		return driver;
	}

	public AwsVideoStore() {
		this.withType(SchemaHub.getType("dcmAwsVideoStore"));
	}

	@Override
	public AwsVideoStore deepCopy() {
		AwsVideoStore cp = new AwsVideoStore();
		this.doCopy(cp);
		return cp;
	}
	...
	public FileStoreFile wrapFileRecord(RecordStruct file) {
		return AwsVideoStoreFile.of(this, file, false);
	}
	...
	public void getFileDetail(CommonPath path, OperationOutcome<FileStoreFile> callback) throws OperatingContextException {
		AwsVideoStoreFile f = AwsVideoStoreFile.of(this, path, false);
		...
		AWSS3.createFolder(connection, region, bucket, dest, new OperationOutcomeEmpty() {
			...
			public void callback() throws OperatingContextException {
				if (this.hasErrors()) {
					...
					AwsVideoStoreFile folder = AwsVideoStoreFile.of(AwsVideoStore.this, path, true);
	...
	public FileStoreFile fileReference(CommonPath path) {
		return AwsVideoStoreFile.of(this, path, false);
	}
	
	@Override
	public FileStoreFile fileReference(CommonPath path, boolean isFolder) {
		return (FileStoreFile) AwsVideoStoreFile.of(this, path, false).withIsFolder(isFolder);
	}
